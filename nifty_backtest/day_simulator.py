...
class DaySimulator:
    ...
    def simulate(self, day: DayData) -> DayResult:
        ...
        result.ce_sl = calculate_sl(
            ...
            vix_low=self.p.vix_low,
            vix_mid_low=self.p.vix_mid_low,
            vix_mid_high=self.p.vix_mid_high,
            sl_pct_r1=self.p.sl_pct_vix_r1,
            sl_pct_r2_calm=self.p.sl_pct_vix_r2_calm,
            sl_pct_r2_volatile=self.p.sl_pct_vix_r2_volatile,
            sl_pct_r3=self.p.sl_pct_vix_r3,
            sl_pct_r4=self.p.sl_pct_vix_r4,
        ...
        for ts in all_times:
            ...
            if ce_open and ce_high is not None:
                ...
                if active_sl and ce_high >= active_sl:
                    ...
                    result.ce_exit_time = time_str; ce_open = False
                    # Immediately close CE hedge — no reason to hold it once sell leg is gone
                    if ceh_open and ceh_close is not None:
                        result.ce_hedge_exit = ceh_close
                        result.ce_hedge_exit_reason = "SELL_LEG_EXIT"; ceh_open = False
                    if not pe_in_trail:
                        pe_in_trail = True
            ...
            if pe_open and pe_high is not None:
                ...
                if active_sl and pe_high >= active_sl:
                    ...
                    result.pe_exit_time = time_str; pe_open = False
                    # Immediately close PE hedge — no reason to hold it once sell leg is gone
                    if peh_open and peh_close is not None:
                        result.pe_hedge_exit = peh_close
                        result.pe_hedge_exit_reason = "SELL_LEG_EXIT"; peh_open = False
                    if not ce_in_trail:
                        ce_in_trail = True
